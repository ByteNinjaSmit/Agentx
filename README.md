...
- **Data sources**: Semantic Scholar API (papers), GDELT / NewsAPI (news), Hacker News Algolia API (social), patents live via USPTO ODP → SerpAPI Google Patents → Google Programmable Search, tried in that order (whichever is configured — SerpAPI is the recommended low-friction option: email signup, 250 free/month; USPTO ODP needs an MFA-verified USPTO.gov account; Google Programmable Search's underlying Custom Search JSON API no longer grants access to new Cloud projects — otherwise curated fixture data, disclosed either way)
...
- **Autonomous fleet behaviors** (`backend/agent/fleet.py`), each a real, bounded runtime decision rather than a prompt-only instruction:
  ...
  - **Loop guard** — a research lane (or the single-loop pipeline) that repeats the identical tool call with no new evidence gets blocked in-run rather than burning its step budget on it; a synthetic tool result tells the model why so it tries something else, surfaced in the trace and in `final["loop_events"]`
...
- **Evaluation harness** (`backend/evaluation/`) — runs the real fleet graph against scripted providers/tools (no network, no DB) across 53 cases in 7 categories (normal/tool-failure/contradictory/incomplete/adversarial/replanning/ambiguous), every case scripted for both `pipeline=fleet` and `pipeline=single`, with 9 deterministic pass/fail checks; `cd backend && python -m evaluation.runner --pipeline both --repeat 3` (318 runs, 714 checks, 100% pass). See [docs/ROADMAP.md § 8](docs/ROADMAP.md#8-evaluation-ladder-6--built-53-case-dataset--llm-judge--human-eval-tiers) for what's still open (CI wiring, collecting human ratings)
- **LLM-as-judge evaluation** — the same scripted cases graded for answer quality (accuracy, groundedness, completeness, evidence quality, uncertainty handling, unsupported-claims count) by a real model via `agent.providers.LLMProvider`, goal/evidence/candidate-answer kept as separate labeled fields so the candidate's own output can't phrase its grading criteria; `cd backend && python -m evaluation.judge_runner` (needs `GEMINI_API_KEY` or `ANTHROPIC_API_KEY`)
- **Human-evaluation protocol** (`backend/evaluation/human_eval.py`, [docs/HUMAN_EVAL_PROTOCOL.md](docs/HUMAN_EVAL_PROTOCOL.md)) — generates a 1-5 rubric rating sheet (accuracy/groundedness/completeness/evidence quality/useful recommendation/uncertainty handling/overall) over one case per category × both pipelines, and scores a filled-in copy (mean per dimension, inter-rater agreement); `cd backend && python -m evaluation.human_eval --generate`. The generated sheet is real and reproducible; no scores have been collected yet — `--score` on it correctly reports zero rated rows rather than fabricating a result
- **Observability closed loop** (`backend/observability/`) — turns a completed run's trace into an automatic root-cause diagnosis, applies one of a small fixed set of policy actions (today: a per-run circuit breaker on `search_papers`), and proves it helped by re-running the same scripted scenario and diffing real metrics; `cd backend && python -m evaluation.full_benchmark` prints a real BEFORE/AFTER table (a representative run: 96% lower elapsed time, same findings, `coverage_ok` unchanged). See [docs/ROADMAP.md § 9](docs/ROADMAP.md#9-observability-closed-loop--built) for what's still open
- **LangGraph-native checkpoint/resume** — the fleet graph can compile with a real checkpointer (`agent/fleet.py::_build_graph(checkpointer=...)`); a fresh graph object resumes an interrupted run from its last completed superstep via `astream(None, config)` instead of restarting. Verified offline with LangGraph's `InMemorySaver` (`tests/test_checkpointing.py`); `backend/checkpoint_demo.py` runs the same proof against a real Postgres (`docker compose up -d db` first) but wasn't confirmed reachable this session
