...
pipeline's final result, and (where needed) the full stream of events from that run,
and returns an `EvalResult` or `None` if the check does not apply to that case.
...
def eval_task_success(case: Case, final: dict, events: list[dict]) -> EvalResult | None:
    expected = case.expect.get("expected_kept_ids")
    if expected is None:
        return None
...
def eval_coverage_ok_matches(case: Case, final: dict, events: list[dict]) -> EvalResult | None:
...
def eval_gap_reported(case: Case, final: dict, events: list[dict]) -> EvalResult | None:
...
def eval_recovery_run_completed(case: Case, final: dict, events: list[dict]) -> EvalResult | None:
...
def eval_refusal_on_insufficient_evidence(case: Case, final: dict, events: list[dict]) -> EvalResult | None:
...
def eval_conflict_detected(case: Case, final: dict, events: list[dict]) -> EvalResult | None:
...
def eval_replanning_triggered(case: Case, final: dict, events: list[dict]) -> EvalResult | None:
    if "expect_replanned" not in case.expect:
        return None
...
def eval_hallucination_rejected(case: Case, final: dict, events: list[dict]) -> EvalResult | None:
...
def evaluate(case: Case, final: dict, events: list[dict]) -> list[EvalResult]:
    results = []
    for fn in ALL_EVALUATORS:
        result = fn(case, final, events)
